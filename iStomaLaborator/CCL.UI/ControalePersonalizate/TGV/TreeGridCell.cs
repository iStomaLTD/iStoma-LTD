            ...
            if (node._grid.ShowLines)
            {
                using (Pen linePen = new Pen(SystemBrushes.ControlDark, 1.0f))
                {
                    ...
                    if (node.Level == 1)
                    {
                        ...
                        //Adaugat de pe net - draw the lines in thick rows (that have the wrap property set to 'true')
                        if (node.HasChildren && node.IsExpanded) { graphics.DrawLine(linePen, (glyphRect.X + 4) + INDENT_WIDTH, cellBounds.Top + cellBounds.Height / 2, (glyphRect.X + 4) + INDENT_WIDTH, cellBounds.Bottom); }
                    }
                    else
                    {
                        if (isLastSibling)
