...
namespace CCL.UI.TGV
{
    ...
    public class TreeGridView : DataGridViewPersonalizat
    {
        ...
        public TreeGridNode CreazaNod(int pIndexImagine)
        {
            TreeGridNode xNod = this.Nodes.Add();
            if (pIndexImagine >= 0)
                xNod.ImageIndex = pIndexImagine;
            return xNod;
        }

        public static void SeteazaFundalNod(TreeGridNode pNod, Color pCuloareFundal)
        {
            if (pCuloareFundal != Color.Empty)
            {
                foreach (DataGridViewCell celula in pNod.Cells)
                {
                    celula.Style.BackColor = pCuloareFundal;
                }
            }
        }
        ...
        public TreeGridNode SelectedNode
        {
            get
            {
                return base.SelectedRow as TreeGridNode;
            }
        }
        ...
        public void AfiseazaUltimaLinie()
        {
            try
            {
                if (base.RowCount > 3)
                {
                    int selectedRowIndex = base.RowCount - 1;
                    if (this.FirstDisplayedScrollingRowIndex + base.DisplayedRowCount(false) <= selectedRowIndex)
                    {
                        this.FirstDisplayedScrollingRowIndex =
                           selectedRowIndex - this.DisplayedRowCount(false) + 1;
                        //base.Refresh();
                        //base.CurrentCell = base.Rows[selectedRowIndex].Cells[0];
                        //base.Rows[selectedRowIndex].Selected = true;
                        //base.ResumeLayout();
                        //this.Visible = true;
                    }

                    //this.FirstDisplayedScrollingRowIndex = base.RowCount - 3;
        ...
        {
            //Nu are sens sa declansam schimbarea selectiei atunci cand distrugem controlul
            if (!this._disposing)
        ...
        public new void IncepeContructieRanduri()
        {
            base.IncepeContructieRanduri();
            this.Nodes.Clear();
        }

        public new void AllowModification(bool pPermiteModificarea)
        {
            ...
            foreach (TreeGridNode Rand in this.Nodes)
            {
                AllowModificationNod(Rand, pPermiteModificarea, contineColoanaInchidere, contineColoanaMotivInchidere, contineColoanaEditare);
            }

            this.Invalidate();
        }

        private void AllowModificationNod(TreeGridNode pNod, bool pPermiteModificarea, bool contineColoanaInchidere, bool contineColoanaMotivInchidere, bool contineColoanaEditare)
        {
            if (contineColoanaEditare)
            {
                if (((DataGridViewImageButtonCell)pNod.Cells[lColoanaEditare]).ImageNormal != null) // nu intotdeauna avem aceasta celula (mai ales in cazul TGV-urilor)
                    InitCelulaEditare(pNod, pPermiteModificarea);
            }

            if (contineColoanaMotivInchidere)
                pNod.Cells[lColoanaMotivInchidere].ReadOnly = !pPermiteModificarea;

            if (pNod.Nodes.Count > 0)
            {
                foreach (TreeGridNode nodCopil in pNod.Nodes)
                {
                    AllowModificationNod(nodCopil, pPermiteModificarea, contineColoanaInchidere, contineColoanaMotivInchidere, contineColoanaEditare);
                }
            }
        }

        public void setVizibilitateNoduriTag<T>(bool pVizibil)
        {
            foreach (TreeGridNode nod in this.Nodes)
            {
                setVizibilitateNodTag<T>(nod, pVizibil);
                if (nod.Tag is T)
                    nod.Visible = pVizibil;
            }
        }

        private void setVizibilitateNodTag<T>(TreeGridNode pNod, bool pVizibil)
        {
            if (pNod.Nodes.Count > 0)
            {
                foreach (TreeGridNode nod in pNod.Nodes)
                {
                    setVizibilitateNodTag<T>(nod, pVizibil);
                    if (nod.Tag is T)
                        nod.Visible = pVizibil;
        ...
        public bool RandulContineValoarea(TreeGridNode pRand, string pTextGeneral)
        {
            ...
            {
                if (!areTextGeneral || (areTextGeneral && CUtil.InlocuiesteDiacritice(Convert.ToString(Celula.Value)).ToUpper().Contains(pTextGeneral)))
                    randVizibil = true;
            }

            if (pRand.Nodes.Count > 0)
            {
                int numarRandVizibil = 0;
                Color culAlter = CCL.UI.IHMStilAplicatie._SDGVFundalAlternant;
                Color culLinieNormala = CCL.UI.IHMStilAplicatie._SDGVFundalNormal;
                foreach (var item in pRand.Nodes)
                {
                    bool randVizibilSubNod = RandulContineValoarea(item, pTextGeneral);
                    item.Visible = randVizibilSubNod;

                    if (randVizibilSubNod)
                    {
                        numarRandVizibil++;
                        item.DefaultCellStyle.BackColor = (numarRandVizibil % 2 != 0) ? culLinieNormala : culAlter;
                        randVizibil = true;
                    }
                }
            }
        ...
        public new int FiltreazaDupaText(string pTextGeneral)
        {
            bool randVizibil = false;
            int numarRandVizibil = 0;
            Color culAlter = CCL.UI.IHMStilAplicatie._SDGVFundalAlternant;
            ...
            {
                randVizibil = RandulContineValoarea(item, pTextGeneral);

                item.Visible = randVizibil;
            ...
            //{
            //    randVizibil = (!areTextGeneral || (areTextGeneral && DataGridViewPersonalizat.RandulContineValoarea(this.Rows[i], pTextGeneral)));
